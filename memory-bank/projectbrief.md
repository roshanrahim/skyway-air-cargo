# Project Brief — SKYWAY Air Cargo & Courier Services Website

## What This Project Is
A complete business website for **SKYWAY Air Cargo and Courier Services**, an authorised agent of **Aramex India Pvt and UPS**, based in Alangad, Ernakulam (Cochin), Kerala.

## Core Goals
1. **Build trust** — customers hand over valuable goods; the site must look professional and verified.
2. **Generate leads** — quote requests, WhatsApp chats, and phone calls are the conversion targets.
3. **Reduce support load** — tracking, restricted items list, and FAQ answer common questions automatically.

## Business Facts (source: JustDial verified listing)
- **Name:** SKYWAY Air Cargo and Courier Services
- **Status:** Authorised Agent of Aramex India Pvt and UPS (wording per owner, 29 Jul 2026)
- **Main Office:** First Floor, V R Complex, Near UC College, Aluva Kottappuram, Ernakulam, Kerala – 683511
- **Branch Office:** Chirangara, Malikampeedika, Alangad, Ernakulam, Kerala – 683511
- **Phones:** +91 96454 87929 (primary, also WhatsApp) · +91 80893 90077
- **GSTIN:** 32ALCPL6861B1ZH
- **Hours:** Open 24 hours, 7 days a week
- **Established:** 2021
- **Rating:** 4.5★ (21 ratings, JustDial verified)
- **Services:** Commercial courier, corporate courier, perishable goods, international air cargo

## Live URLs
- **Website:** https://skywayaircargo.in (primary, HTTPS) · github.io URL redirects
- **Repository:** https://github.com/roshanrahim/skyway-air-cargo

## Brand Requirements (from owner)
- Logo: "SKYWAY" in **red**, bold display font, with an **airplane** integrated
- Blue **verified tick** next to the Aramex authorisation claim
- Red (#E30613) as the primary brand colour
